...
const SearchInput = ({setCode, code = ''}: Props) => {
  // event
  ...
  const Item = ({value}: ItemProps) => (
    <TouchableHighlight
      onPress={onPressNation({value})}
      underlayColor="#4AC1E814"
    >
      <View {...style.item}>
        <Text>{lang === 'en' ? value.code_en : value.code_ko}</Text>
      </View>
    </TouchableHighlight>
  )
    ...
    <>
      ...
      >
        <View {...style.wrapper}>
          ...
          <Text {...style.text}>
            {countryName ? countryName : globalText.countryInput[lang]}
          </Text>
        </View>
      </TouchableOpacity>
      {isIFocus && (
